<?hh // strict

/* HH_IGNORE_ERROR[1002] */
$translations = array(
  'date and time format' =>
    'H:i:s D d/m/Y', //used by date() function
  //Translations for IndexController
  'Facebook CTF' =>
    'Facebook CTF',
  'Conquer the world' =>
    'Захисти Україну',
  'Play' =>
    'Грати',
  'Welcome to the Facebook Capture the Flag Competition. By clicking "Play," you will be entered into the official CTF challenge. Good luck in your conquest.' =>
    'Ласкаво просимо на змагання Capture the Flag. Натискаючи кнопку "Грати", ви братимете участь в офіційному змаганні CTF. Бажаємо успіхів!',
  'Get ready for the CTF to start and access the gameboard now!' =>
    'Приготуйтеся до початку CTF і отримайте доступ до ігрового поля!',
  'Gameboard' =>
    'Ігрове поле',
  'Register Team' =>
    'Зареєструвати гравця',
  'Get ready for the CTF to start and register your team now!' =>
    'Приготуйтеся до початку CTF і зареєструйтеся!',
  'Login' =>
    'Увійти',
  'Soon' =>
    'Незабаром',
  'Upcoming Game' =>
    'Майбутня Гра',
  '_days' =>
    '_дн',
  '_hours' =>
    '_г',
  '_minutes' =>
    '_хв',
  '_seconds' =>
    '_с',
  'Official CTF Rules' =>
    'Офіційні Правила CTF',
  'Following actions are prohibited, unless explicitly told otherwise by event Admins.' =>
    'Наведені нижче дії заборонені, за виключенням випадків, коли адміністратори заходу явно стверджують зворотне.',
  'Rule' =>
    'Правило',
  'Cooperation' =>
    'Взаємодопомога',
  'No cooperation between teams with independent accounts. Sharing of keys or providing revealing hints to other teams is cheating, don’t do it.' =>
    'Забороняється співробітництво між гравцями. Поширення ключів і надання явних підказок іншим гравцям є шахрайством і заборонено.',
  'Attacking Scoreboard' =>
    'Табло атак',
  'No attacking the competition infrastructure. If bugs or vulns are found, please alert the competition organizers immediately.' =>
    'Заборонені спроби атак на інфраструктуру змагання. У разі виявлення багів або вразливостей просимо негайно повідомити про них організаторам.',
  'Sabotage' =>
    'Саботаж',
  'Absolutely no sabotaging of other competing teams, or in any way hindering their independent progress.' =>
    'Строго заборонено шкодити іншим гравцям або яким-небудь чином заважати їх прогресу.',
  'Bruteforcing' =>
    'Атака повним перебором',
  'No brute forcing of challenge flag/ keys against the scoring site.' =>
    'Підбирати прапорці методом перебору на сайті змагання заборонено.',
  'Denial Of Service' =>
    'DoS-атака',
  'DoSing the CTF platform or any of the challenges is forbidden.' =>
    'DoS-атака на платформу CTF або на будь-яке змагання заборонена.',
  'Legal' =>
    'Заява',
  'Disclaimer' =>
    'Disclaimer',
  'By participating in the contest, you agree to release Facebook and its employees, and the hosting organization from any and all liability, claims or actions of any kind whatsoever for injuries, damages or losses to persons and property which may be sustained in connection with the contest. You acknowledge and agree that Facebook et al is not responsible for technical, hardware or software failures, or other errors or problems which may occur in connection with the contest.' =>
    'Беручи участь в змаганні, ви погоджуєтеся з повним звільненням від будь-якої відповідальності компанії Facebook, її співробітників і компанії-організатора заходу, претензій і дій будь-якого характеру за травми, збиток по відношенню до людей, матеріальних цінностей, а також за пропажу власності, що може бути пов\'язано з змаганням.',
  'If you have any questions about what is or is not allowed, please ask an organizer.' =>
    'Якщо у вас є які-небудь питання щодо того, що заборонено або дозволено, будь ласка, зверніться до організатора.',
  'Have fun!' =>
    'Насолоджуйтесь!',
  'Name' =>
    'Ім\'я',
  'Email' =>
    'Email',
  'Token' =>
    'Токен',
  'Team Registration' =>
    'Реєстрація Гравця',
  'Team Name' =>
    'Нікнейм Гравця',
  'Password' =>
    'Пароль',
  'Choose an Emblem' =>
    'Виберіть Емблему',
  'or upload your own' =>
    'або завантажте свою власну',
  'Clear your custom emblem to use a default emblem.' =>
    'Видаліть свою емблему, щоб використовувати емблему за замовчуванням.',
  'Password is too simple' =>
    'Пароль занадто простий',
  'Sign Up' =>
    'Зареєструватися',
  'Register to play Capture The Flag here. Once you have registered, you will be logged in.' =>
    'Зареєструйтесь, щоб зіграти в Capture The Flag. Після завершення реєстрації автоматично буде виконаний вхід в систему.',
  'Not Available' =>
    'Недоступно',
  'Team Registration will be open soon, stay tuned!' =>
    'Реєстрація Гравця буде скоро відкрита, залишайтеся на зв\'язку!',
  'Try Again' =>
    'Спробуйте Знову',
  'Select' =>
    'Вибрати',
  'Team Login' =>
    'Вхід для Гравця',
  'Please login here. If you have not registered, you may do so by clicking "Sign Up" below. ' =>
    'Будь ласка, виконайте вхід. Якщо ви не зареєструвалися, натисніть кнопку "Зареєструватися" нижче.',
  'Team Login will be open soon, stay tuned!' =>
    'Вхід для Гравця буде скоро відкритий, залишайтеся на зв\'язку!',
  'ERROR' =>
    'ПОМИЛКА',
  'Start Over' =>
    'Почати спочатку',
  'Window is too small' =>
    'Вікно занадто маленьке',
  'For the best CTF experience, please make window size bigger.' =>
    'Для найкращих відчуттів від гри збільште розмір вікна.',
  'Thank you.' =>
    'Дякуємо.',
  'Logout' =>
    'Вийти',
  'Registration' =>
    'Реєстрація',
  'Play CTF' =>
    'Зіграти в CTF',
  'Rules' =>
    'Правила',
  //Translations for GameboardController
  'Admin' =>
    'Admin',
  'ADMIN' =>
    'ADMIN',
  'Navigation' =>
    'Навігація',
  'View Mode' =>
    'Режим Перегляду',
  'View mode' =>
    'Режим перегляду',
  'Tutorial' =>
    'Як_користуватися',
  'Scoreboard' =>
    'Табло',
  'You' =>
    'Ви',
  'Others' =>
    'Решта',
  'All' =>
    'Всі',
  'Leaderboard' =>
    'Лідери',
  'Announcements' =>
    'Анонси',
  'Teams' =>
    'Команди',
  'Filter' =>
    'Фільтр',
  'Activity' =>
    'Дії',
  'Game Clock' =>
    'Ігровий Годинник',
  //Translations for AdminController
  'Auto' =>
    'Авто',
  'All Categories' =>
    'Всі категорії',
  'Open' =>
    'Відкрито',
  'Tokenized' =>
    'За токенами',
  'Hour' =>
    'Година',
  'Hours' =>
    'Години',
  'Used by' =>
    'Використовує',
  'Used By' =>
    'Використовує',
  'Available' =>
    'Доступно',
  'Registration Tokens' =>
    'Токени для Реєстрації',
  'Create More' =>
    'Створити Ще',
  'Export Available' =>
    'Експортувати Доступні',
  'Not started yet' =>
    'Ще не почалося',
  'Configuration' =>
    'Налаштування',
  'Tokens' =>
    'Токени',
  'Game Configuration' =>
    'Налаштування гри',
  'OK' =>
    'OK',
  'status_' =>
    'статус_',
  'On' =>
    'Увімк.',
  'Off' =>
    'Вимк.',
  'Player Names' =>
    'Імена Гравців',
  'Players Per Team' =>
    'Гравців в Команді',
  'Registration Type' =>
    'Тип Реєстрації',
  'Strong Passwords' =>
    'Надійні Паролі',
  'Team Selection' =>
    'Вибір Команди',
  'Game' =>
    'Гра',
  'Scoring' =>
    'Заробляння балів',
  'Progressive Cycle (s)' =>
    'Зростаючий цикл (с)',
  'Refresh Gameboard' =>
    'Оновити Ігрове Поле',
  'Default Bonus' =>
    'Бонус за замовчуванням',
  'Bases Cycle (s)' =>
    'Базовий Цикл (с)',
  'Default Bonus Dec' =>
    'Зниження Бонуса по Замовчуванням',
  'Timer' =>
    'Таймер',
  'Server Time' =>
    'Серверний Час',
  'Game Duration' =>
    'Тривалість Гри',
  'Begin Time' =>
    'Початок',
  'Expected End Time' =>
    'Очікуваний Час Закінчення',
  'Internationalization' =>
    'Інтернаціоналізація',
  'Language' =>
    'Мова',
  'Branding' =>
    'Брендінг',
  'Custom Logo' =>
    'Власний Логотип',
  'Logo' =>
    'Логотип',
  'Custom Text' =>
    'Власний Текст',
  'DELETE' =>
    'ВИДАЛИТИ',
  'Delete' =>
    'Видалити',
  'No Announcements' =>
    'Немає Повідомлень',
  'Game Controls' =>
    'Управління Грою',
  'Write New Announcement here' =>
    'Нове повідомлення пишіть тут',
  'Create' =>
    'Створити',
  'General' =>
    'Загальні',
  'Back Up Database' =>
    'Резервна Копія БД',
  'Export Full Game' =>
    'Експортувати Всю Гру',
  'Import Full Game' =>
    'Імпортувати Всю Гру',
  'Import Teams' =>
    'Імпортувати Команди',
  'Export Teams' =>
    'Експортувати Команди',
  'Import Logos' =>
    'Імпортувати Логотипи',
  'Export Logos' =>
    'Експортувати Логотипи',
  'Import Levels' =>
    'Імпортувати Рівні',
  'Export Levels' =>
    'Експортувати Рівні',
  'Import Categories' =>
    'Імпортувати Категорії',
  'Export Categories' =>
    'Експортувати Категорії',
  'Levels' =>
    'Рівні',
  'New Quiz Level' =>
    'Новий Рівень Вікторини',
  'Title' =>
    'Назва',
  'Question' =>
    'Запитання',
  'Level title' =>
    'Назва рівня',
  'Quiz question' =>
    'Запитання вікторини',
  'Country' =>
    'Держава',
  'Answer' =>
    'Відповідь',
  'Points' =>
    'Бали',
  'Hint' =>
    'Підказка',
  'Hint Penalty' =>
    'Штраф за Підказку',
  'EDIT' =>
    'РЕДАГУВАТИ',
  'All Quiz Levels' =>
    'Всі Рівні Вікторини',
  'Filter By:' =>
    'Відфільтрувати За:',
  'All Status' =>
    'Всі статуси',
  'Enabled' =>
    'Увімкнено',
  'Disabled' =>
    'Вимкнено',
  'Quiz Level' =>
    'Рівень Вікторини',
  'Show Answer' =>
    'Показати Відповідь',
  'Bonus' =>
    'Бонус',
  '-Dec' =>
    '-Зменшення',
  'Save' =>
    'Зберегти',
  'Quiz Management' =>
    'Управління Вікториною',
  'Add Quiz Level' =>
    'Додати Рівень Вікторини',
  'New Flag Level' =>
    'Новий Рівень Прапорця',
  'Description' =>
    'Опис',
  'Level description' =>
    'Опис рівня',
  'Category' =>
    'Категорія',
  'Flag' =>
    'Прапорець',
  'flag' =>
    'прапорець',
  'All Flag Levels' =>
    'Всі Рівні з Прапорцем',
  'New Attachment:' =>
    'Нове Вкладення:',
  'Attachment' =>
    'Вкладення',
  'Link' =>
    'Посилання',
  'New Link:' =>
    'Нове Посилання:',
  'Flag Level' =>
    'Рівень Прапорця',
  'Categories' =>
    'Категорії',
  '+ Attachment' =>
    '+ Вкладення',
  '+ Link' =>
    '+ Посилання',
  'Flags Management' =>
    'Управління Прапорцями',
  'Add Flag Level' =>
    'Додати Рівень з Прапорцем',
  'New Base Level' =>
    'Новий Рівень Бази',
  'Keep Points' =>
    'Бали Утримання',
  'Capture points' =>
    'Бали Захищення',
  'All Base Levels' =>
    'Всі Рівні Бази',
  'Base Level' =>
    'Рівень Бази',
  'Bases Management' =>
    'Управління Базами',
  'Add Base Level' =>
    'Додати Рівень Бази',
  'New Category' =>
    'Нова категорія',
  'Category: ' =>
    'Категорія: ',
  'Categories Management' =>
    'Управління Категоріями',
  'Add Category' =>
    'Додати Категорію',
  'All Countries' =>
    'Всі Держави',
  'In Use' =>
    'Використовується',
  'In use' =>
    'Використовується',
  'Not Used' =>
    'Не Використовується',
  'Yes' =>
    'Так',
  'No' =>
    'Ні',
  'ISO Code' =>
    'Код ISO',
  'Countries Management' =>
    'Управління Державами',
  'No Team Names' =>
    'Немає Імен Команд',
  'time' =>
    'час',
  'type' =>
    'тип',
  'pts' =>
    'бал.',
  'Level' =>
    'Рівень',
  'level' =>
    'рівень',
  'No Scores' =>
    'Немає Балів',
  'Attempt' =>
    'Спроба',
  'No Failures' =>
    'Немає Невдач',
  'Team' =>
    'Команда',
  'team' =>
    'команда',
  'Names' =>
    'Імена',
  'Scores' =>
    'Бали',
  'Failures' =>
    'Невдачі',
  'New Team' =>
    'Нова Команда',
  'Team Logo' =>
    'Логотип Команди',
  'Selected Logo:' =>
    'Обраний Логотип:',
  'Select Logo' =>
    'Обрати Логотип',
  'All Teams' =>
    'Всі Команди',
  'Protected' =>
    'Захищено',
  'Score' =>
    'Рахунок',
  'Change Password' =>
    'Змінити Пароль',
  'Admin Level' =>
    'Рівень Адміністратора',
  'Visibility' =>
    'Видимість',
  'Team Management' =>
    'Управління Командою',
  'Add Team' =>
    'Додати Команду',
  'None' =>
    'Ніхто',
  'Logo Name' =>
    'Назва Логотипу',
  'Logo Management' =>
    'Управління Логотипом',
  'Session' =>
    'Сесія',
  'Cookie' =>
    'Cookie',
  'Creation Time' =>
    'Час Створення',
  'Last Access' =>
    'Останній Доступ',
  'Last Page Access' =>
    'Останній доступ до сторінки',
  'Data' =>
    'Дані',
  'Sessions' =>
    'Сесії',
  'entry' =>
    'запис',
  'No Entries' =>
    'Немає Записів',
  'Game Logs' =>
    'Журнали Гри',
  'Game Logs Timeline' =>
    'Журнали Гри за Часом',
  'End Game' =>
    'Завершити Гру',
  'Begin Game' =>
    'Почати Гру',
  'Game Admin' =>
    'Адміністратор Гри',
  'Controls' =>
    'Управління',
  'Quiz' =>
    'Вікторина',
  'Flags' =>
    'Прапорці',
  'Bases' =>
    'Бази',
  'Countries' =>
    'Держави',
  'Logos' =>
    'Логотипи',
  //Translations for inc/* and inc/gameboard/*
  'captured' =>
    'захищено',
  'Status' =>
    'Статус',
  'Completed' =>
    'Завершено',
  'Remaining' =>
    'Залишається',
  'Start' =>
    'Початок',
  'End' =>
    'Кінец',
  'Rank' =>
    'Ранг',
  'pts' =>
    'бали', //points
  'Your Rank' =>
    'Ваш Ранг',
  'Your Score' =>
    'Ваш Рахунок',
  'Everyone' =>
    'Всі',
  'Your Team' =>
    'Ваша Команда',
  'Captured' =>
    'Захищено',
  'Initiating' =>
    'Ознайомлення',
  'run : > boot_sequence' =>
    'run : > boot_sequence',
  'Extracting' =>
    'Вилучення',
  //Translations for Utils.php's time_ago() function
  'just now' =>
    'тілько що',
  'd' =>
    'д', //day
  'hr' =>
    'г', //hour
  'min' =>
    'хв', //minute
  'sec' =>
    'с', //second
  'ds' =>
    'дн', //days
  'hrs' =>
    'г', //hours
  'mins' =>
    'хв', //minutes
  'secs' =>
    'с', //seconds
  'ago' =>
    'тому',
  //Translations for ModalControllers
  'begin_' =>
    'почати_',
  'Are you sure you want to kick off the game? Logs will be cleared and progressive scoreboard will start' =>
    'Ви впевнені, що хочете почати гру? Журнали будуті очищенні, почнеться послідовне нарахування балів',
  'end_' =>
    'кінец_',
  'Are you sure you want to finish the current game?' =>
    'Ви впевнені, що хочете завершити поточну гру?',
  'Are you sure you want to logout from the game?' =>
    'Ви впевнені, що хочете вийти з гри?',
  'Saved' =>
    'Збережено',
  'All changes have been successfully saved.' =>
    'Всі зміни успішно збережені.',
  'Error' =>
    'Помилка',
  'Sorry your form was not saved. Please correct the all errors and save again.' =>
    'Вибачте, ваша форма не була збережена. Будь ласка, виправте всі помилки і збережіть ще раз.',
  'cancel_' =>
    'скасувати_',
  'Are you sure you want to cancel? You have unsaved changes that will be reverted.' =>
    'Ви впевнені, що бажаєте скасувати? У вас є незбережені зміни, які будуть втрачені.',
  'choose_logo' =>
    'вибрати_лого',
  'captured_' =>
    'захищено_',
  'flag_owner_' =>
    'власник_прапорця_',
  'INACTIVE' =>
    'НЕ АКТИВНО',
  'PTS' =>
    'БАЛИ',
  'category' =>
    'категорія',
  'capture_' =>
    'захищення_',
  'Insert your answer' =>
    'Введіть свою відповідь',
  'Request Hint' =>
    'Запросити Підказку',
  'Submit' =>
    'Надіслати',
  'hint_' =>
    'підказка_',
  'first_capture' =>
    'перше_захищення',
  'completed_by' =>
    'виконано',
  'scoreboard_' =>
    'табло_',
  'filter_' =>
    'фільтр_',
  'rank_' =>
    'ранг_',
  'team_name_' =>
    'назва_команди_',
  'quiz_pts_' =>
    'бали_вікторини_',
  'flag_pts_' =>
    'бали_прапорця_',
  'base_pts_' =>
    'бали_бази_',
  'total_pts_' =>
    'всього_балів_',
  'team_' =>
    'команда_',
  'team_members' =>
    'члени_команди',
  'base_pts' =>
    'бали_бази',
  'quiz_pts' =>
    'бали_вікторини',
  'flag_pts' =>
    'бали_прапорця',
  'total_pts' =>
    'всього_балів',
  'Tool bars are located on all edges of the gameboard. Tap a category to expand and close each tool bar.' =>
    'На всіх краях ігрового поля розташовані панелі інструментів. Виберіть категорію, щоб розгорнути і згорнути відповідну панель інструментів.',
  'Tool_Bars' =>
    'Панелі_Інструментов',
  'Tap the "Game Clock" to keep track of time during gameplay. Don’t let time get the best of you.' =>
    'Натисніть на "Ігровий Годинник", щоб стежити за часом під час гри. Не дозволяйте часу застати себе зненацька.',
  'Game_Clock' =>
    'Ігровий_Годинник',
  'Countries marked with an ' =>
    'Країни, відмічені',
  'are captured by you.' =>
    ' захищені вами.',
  ' are owned by others.' =>
    ' були захищені іншим.',
  'Captures' =>
    'Захищення',
  'Tap Plus[+] to Zoom In. Tap Minus[-] to Zoom Out.' =>
    'Натисніть Плюс[+], щоб Збільшити масштаб. Натисніть Мінус[-], щоб Зменшити масштаб.',
  'Click and Drag to move left, right, up and down.' =>
    'Натисніть і перетягніть, щоб пересунути вліво, вправо, вгору або вниз.',
  'Zoom' =>
    'Масштабування',
  'Tap Forward Slash [/] to activate computer commands. A list of commands can be found under "Rules".' =>
    'Використовуйте Слеш[/], щоб активувати комп"ютерні команди. Список команд можна знайти в "Правилах".',
  'Command_Line' =>
    'Командний_Рядок',
  'Click "Nav" to access main navigation links like Rules of Play, Registration, Blog, Jobs & more.' =>
    'Натисніть на "Навігацію", щоб отримати основні навігаційні посилання на зразок Правил Гри, Реєстрації, Блогу, Вакансій та ін.',
  'Track your competition by clicking "scorboard" to access real-time game statistics and graphs.' =>
    'Слідкуйте за ходом змагання за допомогою "табло", де можна переглядати статистику гри і графіки в режимі реального часу.',
  'Have fun, be the best and conquer the world.' =>
    'Насолоджуйтеся грою, будьте кращими і захистіть Україну.',
  'Game_On' =>
    'Гра_Увімк',
  'tutorial_' =>
    'як_користуватися_',
  'Next' =>
    'Наст.',
  'Skip to play' =>
    'Пропустити',
  'Powered By Facebook' =>
    'За підтримки Facebook',
  'Active Directory / LDAP' =>
    'Active Directory / LDAP',
  'LDAP Server' =>
    'Сервер LDAP',
  'LDAP Port' =>
    'Порт LDAP',
  'LDAP Domain' =>
    'Домен LDAP',
);
